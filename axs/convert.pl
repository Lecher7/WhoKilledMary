...
Usage:
	perl convert.pl eye-to-ncsa < infile > outfile

Will convert an EyeSoft log on standard input (infile) into an NCSA Extended log in standard output (to outfile).

Usage:
	perl convert.pl -s:www.aftk.com ncsa-to-eye < infile > outfile

Will convert an NCSA Extended log on standard input (infile) to an EyeSoft log in standard output (to outfile).
...
Flags:
	...
	-s:servername
	The hostname of the web server that generated the inputted NCSA log.  Defaults to www.aftk.com
...
Any quotes in the raw EyeSoft data will be stripped when writing to the NCSA format (like a quote in a URL)
...
my $servername = 'www.whokilledmary.com';
...
Err: {
	...
	foreach (@ARGV) {
		if (m!^-v$!) { $b_verbose = 1 }
		if (m!^-d$!) { $b_dns = 1 }
		if (m!^eye-to-ncsa$!) { $direction = 1 }
		if (m!^ncsa-to-eye$!) { $direction = 2 }
		if (m!^-s:(.*)$!) { $servername = $1 }
		}
	...
	if ($direction == 2) {
		...
		# NCSA log converted to EyeSoft format
		...
		# EyeSoft log converted to NCSA format
		...
		while (defined($_ = <STDIN>)) {
			...
			unless ($#Fields > 8) {
				print STDERR "Input line $linenum '" . substr(&Trim($_), 0, 40) . "' does not pattern match to the EyeSoft log format.\n";
				next;
				}
			...
			if ($Fields[4] =~ m!://([^/]+)/(.*)$!) {
				...
				print STDERR "Unable to parse EyeSoft log line $linenum\n";
				next;
				}
