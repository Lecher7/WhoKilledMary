...
EyeSite Script Set, Administration Module
Copyright 2002 by Eye Software Solutions
...
Please adhere to the copyright notice and conditions of use as described at the URL below.  For latest version and help files, visit:
	http://www.aftk.com/scripts/EyeSite/
...
# You should place the log.txt and eyesite.dat files in the same directory as
...
#	Other examples:
#	$LogFile = '/usr/www/users/eyesite/log.txt';
#	$LogFile = 'c:/eyesite/log.txt';



# Enter your anchor page. This will form a link at the top of each EyeSite
# output document:
...
# Protect EyeSite with a username and password.  Both are case sensitive.  You
...
$AllowAnonymousForGraphs = 1; # set to 1 to allow
...
$Admin_Email_Address = &query_env('SERVER_ADMIN', 'nobody@localhost');
#	Example:
#	$Admin_Email_Address = 'president@whitehouse.gov';

# Your favorite network lookup services:
$nslookup = 'http://www.arin.net';
$whois = 'http://www.arin.net';

# Alternate (previous) whois script was:
# $whois = 'http://www.networksolutions.com/cgi-bin/whois/whois?';

# EyeSite can collapse web addresses which include the default document.
...
# their URL.  For example, visits to http://www.aftk.com/ would show up in
...
%LocalAddressTitlePairs = (
	'http://www.whokilledmary.com/' , 'Home Page',
	'http://www.whokilledmary.com/scripts/' , 'Scripts Page',
	'http://www.whokilledmary.com/scripts/axs/' , 'EyeSite Script Page',
	);
...
%GraphOptions = (
	...
	's05' => 'Visitor 1-level Domain (aftk.com)',
	's06' => 'Visitor Full (dialup-123.aftk.com)',
	's07' => 'Visitor IP Address (24.44.243.3)',
...
<HTML>
<HEAD>
	<TITLE>EyeSite Script Set - Administrator's Page</TITLE>
...
<CENTER>
	<FONT SIZE="+1">EyeSite by Eye Software Solutions<BR></FONT>
...
<b>EyeSite</b> keeps records on visits to your site. This companion script, <TT>AX-ADMIN</TT>, allows you to display these records in meaningful graph and database formats.  We currently have <B>$cur_hits</B> hits to work with.
...
<P>Enter the number of recent hits you'd like to view, or leave blank for all. Enter "L" to view hits since your last visit on $PREF{'last_string'}.</P>
...
<INPUT TYPE="reset" VALUE="Clear"><INPUT TYPE="button" VALUE="Set Defaults" NAME="defaults" ONCLICK="setdefs()"><BR>
<INPUT TYPE="submit" VALUE="View in Graphical Format" NAME="MakeGraphs" ONCLICK="JavaMakeGraphs()"><BR>
	<IMG SRC="$PREF{'images_folder'}EyeLogo.jpg" ALT="EyeSite logo" HEIGHT="205" WIDTH="198">
</TD>
</TR>
</TABLE>
</CENTER>
...
<BLOCKQUOTE>
	<P>By default, <b>EyeSite</b> will graph all hits in the database.
	...
	<P>The filter string may contain a file name, server name, or browser type - if this field is used, all graphs will be designed from log entries with contain this string as a pattern match.</P>
</BLOCKQUOTE>
</FORM>
...
EOM
	...
	foreach ('pl','cgi') {
		if (-e "ax.$_") {
			print "<P><B>See Also:</B></P><BLOCKQUOTE><P>Getting Started: <A HREF=\"ax.$_?debugme\">Instructions for Tagging HTML Pages</A></P></BLOCKQUOTE>";
			last;
			}
		}
...
$webmaster_logging = <<"EOM";
	...
	<P>Currently, your visits <B>ARE NOT</B> being logged.  This is the recommended state.  To begin logging yourself again, click here: <B><A HREF="$This_Script_Address?Target=Preferences&SetCookie=1&CookieValue=0">Log My Visits</A></B>.</P>
...
$webmaster_logging = <<"EOM";
	...
	<P>Currently, your visits <B>are</B> being logged.  To stop logging yourself (recommended), click here: <B><A HREF="$This_Script_Address?Target=Preferences&SetCookie=1&CookieValue=1">Do Not Log My Visits</A></B>.</P>
...
<P>Because you'll typically generate the same graphs repeatedly, <b>EyeSite</b> allows you to specify default settings. Enter your most common settings below. Later, <TT>AX-ADMIN</TT> will select these values automatically.</P>
...
<BLOCKQUOTE>
	...
	<P>By default, <b>EyeSite</b> will graph all hits in the database. However, with these filters, you can restrict graphs to recent hits, critical files, or both.</P>
...
<DD>http://www.aftk.com/links.html#localsites<I>
becomes</I><BR>http://www.aftk.com/links.html<BR></DD>
...
<DD>http://www.aftk.com/links.cgi?foo=bar<I>
becomes</I><BR>http://www.aftk.com/links.cgi<BR></DD>
...
<DD>http://www.aftk.com/$DefaultDoc<I>
becomes</I><BR>http://www.aftk.com/</DD>
...
<P>The URL to the folder containing the "red.gif" and "tracker.jpg" images: <INPUT NAME="images_folder" SIZE=40><BR>
You can use http://www.aftk.com/images/ if you want. There is no guarantee that the images will always be hosted there, so it is best if you can host the images on your own server.</P>
...
<BLOCKQUOTE>
	...
	<P>This feature requires that you have cookies enabled.  Selecting <B>Do Not Log My Visits</B> will set a cookie to your browser that tells the ax.pl script to not log your visits.  You will need to do this for each browser you use, and you will need to repeat this process every time you delete cookies.</P>
...
<P>This script is operated by <A HREF="mailto:$Admin_Email_Address">$Admin_Email_Address</A>, or a third party reachable via that address. Contact them for help with usernames and passwords. Technical issues, bug reports, and the like may be directed to <A HREF="mailto:EyeSoft\@optonline.net"><FONT SIZE="-1">EyeSoft\@optonline.net</FONT></A>.</P>
...
sub AuthPref {
	...
	Err: {
		...
		%PREF = (
			...
			'images_folder' => 'http://www.aftk.com/images/',
			'Filter', '',
			'recent', '',
			'NumSort', 'CHECKED',
			'NewWindow', 'CHECKED',
...
<P><B>EyeSite Debug Screen</B></P>

<P>This is one tool, of many, to help you out. Read the <A HREF="http://www.aftk.com/scripts/eyesite/" TARGET="_blank">trouble-shooting guide</A> if you need more detailed assistance.</P>
...
<H5 ALIGN="center">Visit <A HREF="http://www.aftk.com/scripts/eyesite/" TARGET="_blank">the EyeSite help page</A> for more information.  EyeSite is copyright 2002 by Eye Software Solutions.</H5>
...
# <FORM METHOD="post" ACTION="http://www.aftk.com/bug.pl">
